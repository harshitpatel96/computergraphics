...
var vNormAttribLoc; // where to put position for vertex shader
...
function initTexture(img_name) {
    var img_path = "https://ncsucgclass.github.io/prog4/" + img_name;
...
function loadModels() {
    ...
    try {
        if (inputTriangles == String.null)
			...
			inputTriangles.textureObject1 = initTexture("lightmap.gif");
            for (var whichSet=0; whichSet<numTriangleSets; whichSet++) { // for each tri set
                var depthTotal = 0; // sum the z value
				...
				inputTriangles[whichSet].textureCoor = [];
                inputTriangles[whichSet].depthNum  = 0;
				
				if(inputTriangles[whichSet].alpha == 1) numOpaTri++; // calculate num of opaque objects
                ...
                for (whichSetVert=0; whichSetVert<numVerts; whichSetVert++) { // verts in set
                    ...
                    depthTotal += inputTriangles[whichSet].vertices[whichSetVert][2]; // put z value in
					
                    inputTriangles[whichSet].glVertices.push(vtxToAdd[0],vtxToAdd[1],vtxToAdd[2]); // put coords in set coord list
                    inputTriangles[whichSet].glNormals.push(normToAdd[0],normToAdd[1],normToAdd[2]); // put normal in set coord list
					inputTriangles[whichSet].textureCoor.push(uvToAdd[0], uvToAdd[1]);
					
                    vec3.max(maxCorner,maxCorner,vtxToAdd); // update world bounding box corner maxima
                    vec3.min(minCorner,minCorner,vtxToAdd); // update world bounding box corner minima
                    vec3.add(inputTriangles[whichSet].center,inputTriangles[whichSet].center,vtxToAdd); // add to ctr sum
                } // end for vertices in set
				inputTriangles[whichSet].depthNum = depthTotal / numVerts; // average the depth number, and put in
                ...
                gl.bufferData(gl.ARRAY_BUFFER,new Float32Array(inputTriangles[whichSet].textureCoor),gl.STATIC_DRAW); // data in
...
function setupShaders() {
    ...
    var fShaderCode = `
		...
		uniform bool unlit;  // unlit toggle
		uniform bool lightmaps;  // lightMap toggle
		
		uniform sampler2D utexture0;
		uniform sampler2D utexture1;
        uniform float uAlpha; //for transparent
        ...
        void main(void) {
			...
			highp vec4 color0 = texture2D(utexture0, vec2(vTextureCoord.x, vTextureCoord.y));
			highp vec4 color1 = texture2D(utexture1, vec2(vTextureCoord.x, vTextureCoord.y));
			
			if(Blinn_Phong) { //use light and transparent
                gl_FragColor = vec4(color0.rgb * colorOut, color0.a * uAlpha);    
            }
            else if(lightmaps) { // dont use light
                highp vec4 color01 = color0 * color1;
				gl_FragColor = vec4(color01.rgb, color01.a * uAlpha);
            }
			else if(unlit)
				gl_FragColor = vec4(color0.rgb, color0.a * uAlpha);
        }
    `;
    ...
    try {
        ...
        if (!gl.getShaderParameter(fShader, gl.COMPILE_STATUS)) { // bad frag shader compile
            ...
            if (!gl.getProgramParameter(shaderProgram, gl.LINK_STATUS)) { // bad program link
				...
				vTextAttribLoc = gl.getAttribLocation(shaderProgram, "aTextureCoord");
                gl.enableVertexAttribArray(vTextAttribLoc);
				...
				unlitULoc = gl.getUniformLocation(shaderProgram, "unlit");
				lightmapsULoc = gl.getUniformLocation(shaderProgram, "lightmaps");
                texture0 = gl.getUniformLocation(shaderProgram, "utexture0");
				texture1 = gl.getUniformLocation(shaderProgram, "utexture1");
...
function renderModels() {
    ...
    for (var whichTriSet=0; whichTriSet<numTriangleSets; whichTriSet++) {
		...
		texture = currSet.material.texture;
		...
		gl.uniform1i(unlitULoc, unlit);
		gl.uniform1i(lightmapsULoc, lightmaps);
		...
		gl.vertexAttribPointer(vTextAttribLoc, 2, gl.FLOAT, false, 0, 0);
		...
		gl.bindTexture(gl.TEXTURE_2D, inputTriangles.textureObject1);
